...
/// Synchronize the currently active account credentials to Codex auth.json.
///
/// Purpose:
///     Rewrites the official Codex `auth.json` from the active account stored by Codex Switcher.
/// Inputs:
///     None. The active account ID is read from the local account store.
/// Output:
///     Returns `Ok(())` when the active credentials have been written.
/// Errors:
///     Returns an error string when no active account exists, the active account is missing from
///     storage, or the auth file cannot be written.
/// Side effects:
///     Writes `auth.json` under the resolved Codex home directory.
#[tauri::command]
pub async fn sync_active_account_auth() -> Result<(), String> {
    let active = get_active_account()
        .map_err(|e| e.to_string())?
        .ok_or_else(|| "No active account to sync".to_string())?;

    switch_to_account(&active).map_err(|e| e.to_string())?;
    Ok(())
}
