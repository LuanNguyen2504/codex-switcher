...
/// Checks for running Codex processes through the Tauri command interface.
///
/// Purpose:
///   Lets the frontend decide whether manual account switching should be blocked while Codex is
///   running.
/// Inputs:
///   None.
/// Output:
///   Returns `CodexProcessInfo` with active process count, background process count, switch
///   permission, and active process IDs.
/// Errors:
///   Returns a string error when the process list cannot be queried.
/// Side Effects:
///   Reads the operating system process table.
#[tauri::command]
pub async fn check_codex_processes() -> Result<CodexProcessInfo, String> {
    get_codex_process_info().map_err(|e| e.to_string())
}

/// Checks for running Codex processes from native backend code.
///
/// Purpose:
///   Shares the same process detection rule between frontend commands and native menu bar manual
///   switch actions.
/// Inputs:
///   None.
/// Output:
...
///   permission, and active process IDs.
/// Errors:
///   Returns an error when the process list cannot be queried.
/// Side Effects:
///   Reads the operating system process table.
pub fn get_codex_process_info() -> anyhow::Result<CodexProcessInfo> {
...
/// Finds running Codex processes and separates active blockers from background helpers.
///
/// Purpose:
///   Implements the platform-specific process scan used to decide whether manual account switching
///   is safe.
/// Inputs:
///   None.
/// Output:
///   Returns a tuple of active blocking PIDs and ignored background helper count.
/// Errors:
///   Returns an error when a platform-specific process query fails.
/// Side Effects:
///   Reads the operating system process table.
